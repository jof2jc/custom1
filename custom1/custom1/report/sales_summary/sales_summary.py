...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	for inv in invoice_list:
		...
		row = [
			inv.customer, inv.grand_total, inv.outstanding_amount
		]
...
def get_sales_summary(filters, additional_query_columns):
	...
	return frappe.db.sql("""select customer, sum(grand_total) as grand_total, sum(outstanding_amount) as outstanding_amount
		from `tabSales Invoice`
		where docstatus = 1 %s group by customer order by customer asc""" %
