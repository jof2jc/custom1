...
frappe.ImportInvoiceTool = Class.extend({
	...
	make: function() {
		...
		$(frappe.render_template("import_invoices_main", this)).appendTo(this.page.main);
		...
		this.select.on("change", function() {
			...
			frappe.model.with_doctype(me.doctype, function() {
				...
				if(me.doctype) {
					...
					$(frappe.render_template("import_invoice_tool_columns", {doctype_list: doctype_list}))
		...
		var get_template_url = '/api/method/custom1.custom1.page.import_invoice_tool.exporter.get_template';
	...
	make_upload: function() {
		...
		frappe.upload.make({
			...
			args: {
				method: 'custom1.custom1.page.import_invoice_tool.importer.upload',
			},
			allow_multiple: 0,
			onerror: function(r) {
				me.onerror(r);
...
frappe.pages['import-invoice-tool'].on_page_load = function(wrapper) {
	frappe.import_invoice_tool = new frappe.ImportInvoiceTool(wrapper);
}

frappe.pages['import-invoice-tool'].on_page_show = function(wrapper) {
	frappe.import_invoice_tool && frappe.import_invoice_tool.set_route_options();
}
